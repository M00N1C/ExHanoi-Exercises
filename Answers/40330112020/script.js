...
function exHanoi_2(A, B, C, D, n) {
    const disksOnC = rods[C].children;
    for (let i = 0; i < disksOnC.length; i++) {
        disksOnC[i].style.backgroundColor = "#777";
        disksOnC[i].style.color = "#fff";
    }
    hanoi(A, B, D, n);
    hanoi(C, B, A, n);
    hanoi(D, B, C, n);
...
function exhanoi_5(A, B, C, D, n) {
    if (n == 1) {
        ...
        moves.push([A, D]);
        exhanoi_5utils(C, B, A, D, 6 * (n-1) + 3);
        moves.push([D, C]);
        moves.push([B, C]);
        moves.push([C, D]);
        moves.push([B, C]);
        moves.push([D, C]);
        exhanoi_5utils(A, B, C, D, 6 * (n-1) + 3);
    }
}

function exhanoi_5utils(A, B, C, D, n){
    if (n == 1) {
        moves.push([A, B]);
        moves.push([B, C]);
    }
    else {
        exhanoi_5utils(A, B, C, D, n-1);
        moves.push([A, B]);
        exhanoi_5utils(C, D, A, B, n-1);
        moves.push([B, C]);
        exhanoi_5utils(A, D, C, B, n-1);
    }
}
